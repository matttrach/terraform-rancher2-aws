...
Options:
  -r, --repo OWNER/REPO   The GitHub repository (default: rancher/terraform-rancher2-aws).
...
get_upstream_repo() {
  ...
  # If no upstream, try origin remote
  if [[ -z "${url}" ]]; then
    url=$(git remote get-url origin 2>/dev/null || git config --get remote.origin.url 2>/dev/null || echo "")
  fi

  # If no remote URL found, default to canonical rancher repository
  if [[ -z "${url}" ]]; then
    echo "rancher/terraform-rancher2-aws"
    return
  fi

  # Parse the owner/repo from URL
  local clean_url="${url}"
